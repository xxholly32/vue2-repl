...
function processFile(
  ...
  let [js, importedFiles] = file.filename.endsWith('.vue') ? processVueModule(
    store,
    file.compiled.js,
    file.filename,
...
// 针对vue2文件做单独处理
function processVueModule(
  store: Store,
  script: any,
  filename: string,
  template: string
): [string, Set<string>] {
  const src = script.content
  const s = new MagicString(src)
  ...
  for (const node of ast) {
    ...
    if (node.type === 'ExportDefaultDeclaration') {
      if ('id' in node.declaration && node.declaration.id) {
        ...
        // vue2 特殊处理，需要加入 components 以及其他字段
        if (template) {
          let templateString = `${'\n'} render,${'\n'} staticRenderFns,${'\n'}`
          if (importedFiles.size > 0) {
            templateString += "components:{" + '\n'
            for (const importKey of importedFiles) {
              if (importKey.endsWith('.vue')) {
                templateString += importKey.replace(/\.vue/, "") + ": " + importToIdMap.get(importKey) + '.default,\n'
              }
            }
            templateString += "}," + '\n'
          }
          s.overwrite(node.start!, node.start! + 16, `${template + '\n\n'}${moduleKey}.default = {${templateString}`)
        } else {
          // anonymous default exports
          s.overwrite(node.start!, node.start! + 14, `${moduleKey}.default =`)
        }
  ...
  ; (walk as any)(ast, {
    ...
    ; (walk as any)(ast, {
      enter(node: Node, parent: Node) {
        if (node.type === 'Identifier' && node.name === '__sfc') {
          s.overwrite(node.start!, node.end!, 'answer');
        }
      }
    })
...
// 转换vue依赖组件
export function processVueComponent(importMap: any): string[] {
  let res = ['', '']
  const { imports, components } = importMap
  if (components) {
    for (const key of components) {
      if (imports[key].endsWith('.js') || imports[key].endsWith('.mjs')) {
        res[0] += `import ${key} from "${key}";\n`
        res[1] += `Vue.use(${key});\n`
      }
    }
  }
