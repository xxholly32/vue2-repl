...
import vue2srcdoc from './vue2srcdoc.html?raw'
import { PreviewProxy } from './PreviewProxy'
import { compileModulesForPreview, processVueComponent } from './moduleCompiler'
...
watch(() => store.state.vueEsmUrl, createSandbox)
...
function createSandbox() {
  ...
  if (!importMap.imports.vue) {
    importMap.imports.vue = store.state.vueEsmUrl
  }
  let stylessheets = ""

  if (importMap.css && importMap.css.length > 0) {
    importMap.css.forEach((url: string) => {
      stylessheets += `<link rel="stylesheet" href="${url}">\n`
    });
  }
  delete importMap.components
  delete importMap.css

  const sandboxSrc = vue2srcdoc.replace(
    /<!--IMPORT_MAP-->/,
    JSON.stringify(importMap)
  ).replace(/<!--IMPORT_CSS_MAP-->/, stylessheets)
...
async function updatePreview() {
  ...
  try {
    ...
    if (mainFile.endsWith('.vue')) {
      const ssrModules = compileModulesForPreview(store)
      console.log(
    ...
    if (mainFile.endsWith('.vue')) {
      // 添加 import 以及 vue.use
      let vueUse = ['', '']
      try {
        const raw = store.state.files['import-map.json'].code

        vueUse = processVueComponent(JSON.parse(raw))
      } catch (e: any) {
        store.state.errors = [e as Error]
        return
      }
